...
fn main() -> Result<()> {
    ...
    clone(&samples, &out_dir, true)?;
    clone(&samples, &out_dir, false)?;

    Ok(())
}

fn clone(samples: &[(PathBuf, PathBuf)], out_dir: &Path, low_quality: bool) -> Result<()> {
    let api = match low_quality {
        true => ApiBuilder::from_env()
            .with_progress(true)
            .build()?
            .model(REPO_LOW.into()),
        false => ApiBuilder::from_env()
            .with_progress(true)
            .build()?
            .model(REPO_HIGH.into()),
    };
    api.get(CONFIG)?;
    api.get(VOCAB)?;
    api.get(MERGES)?;
    ...
    for (path_wav, path_txt) in samples.iter() {
        ...
        let name = path_wav
            ...
            .to_owned();
        let out_name = match low_quality {
            true => name + "_low",
            false => name + "_high",
        };
        println!("Cloning \"{out_name}\" ...");
        let out_file = out_dir.join(out_name).with_extension("vox");
